"""Populate the approved LORE v4 template without regenerating its design.
...
ASSETS=MASTER.parent/'front-v3'
from badge_spacing import apply_badge_spacing
...
def reference_data(index):
    cards=json.loads((ASSETS/'source/asmongold-reference-data.json').read_text())
    c=cards[index-1]
    ...
    data.update(rarity=c['rarity'].lower(),artwork=str(ASSETS/c['reference_art']),qr_url=c['qr_demo_payload'])
    return data
...
def render(data,out,demo=False):
    ...
    apply_badge_spacing(root,rarity)
    by_id={n.get('id'):n for n in root.iter() if n.get('id')}
    for node,key in FIELDS.items():
        ...
        font=ImageFont.truetype(str(ASSETS/'source/fonts'/font_name),int(n.get('font-size')))
    ...
    env=os.environ.copy();env['FONTCONFIG_FILE']=str(ASSETS/'source/fontconfig.xml')
