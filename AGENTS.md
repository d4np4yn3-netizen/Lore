...
Read `PROJECT-MEMORY.md` first for the current decisions and exact selected artwork. Repository memory is authoritative when old chat recollections conflict with it.
...
Read `operations/14-STEAK-ARTWORK-CONTINUATION-2026-09-12.md` for the historical recovery handover and `operations/14-STEAK-AND-STYLE-REFERENCE-2026-09-12.md` for its resolution. The exact selected image has been recovered and registered; do not substitute older review files. Daniel subsequently authorised image placement adjustments for the complete card. The current delivered composition is `cards/creators/asmongold/steak-master-01/LORE-Asmongold-Steak-Final-v1.png`, with the exact approved art translated upward 70 SVG units to clear the potato from the fixed QR. No scaling, regeneration or fixed-layout changes. Original art approval, delivered composition confirmation and print release remain separate.
